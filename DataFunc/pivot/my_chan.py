...
def parse2ChanFen(chanK):
    ...
    for i, dt in enumerate(chanK.index):
        ...
        # 顶分型,这里i代表行，使用loc访问
        print(i)
        prehigh = chanK.loc[i-1, 'high']
        curhigh = chanK.loc[i, 'high']
        nexthigh = chanK.loc[i+1, 'high']
        if nexthigh<curhigh>prehigh:
            appendFen(1, i)
        # 底分型
        prelow = chanK.loc[i-1, 'low']
        curhigh = chanK.loc[i, 'low']
        newhigh = chanK.loc[i+1, 'low']
        if newhigh > curhigh < prelow:
            appendFen(-1, i)
